...
function showWelcomeCard() {
  ...
  card.innerHTML = '<span style="font-size:1.5rem;flex-shrink:0;">' + (emojis[v]||'💜') + '</span>' +
    '<div style="flex:1;">' +
    '<div style="font-size:0.9rem;color:var(--text-primary);line-height:1.4;font-weight:500;font-family:-apple-system,BlinkMacSystemFont,Segoe UI Emoji,Apple Color Emoji,Noto Color Emoji,sans-serif;">' + greeting + '</div>' +
    '</div>' +
...
/* =============================================
   ORDER NOTES + NURSE ASSISTANCE
   ============================================= */
function toggleNurseAssist() {
  const track = document.getElementById('nurseToggleTrack');
  const thumb = document.getElementById('nurseToggleThumb');
  const chk   = document.getElementById('nurseToggle');
  const note  = document.getElementById('nurseToggleNote');
  if (!track) return;
  const on = !chk.checked;
  chk.checked = on;
  track.style.background = on ? '#A855F7' : 'var(--surface-3,#333)';
  ...
  if (on) showToast('Staff assistance requested — add notes above');
  state.nurseAssist = on;
}

function getOrderNotes() {
  const inp = document.getElementById('orderNotesInput');
...
function placeOrder() {
  ...
  const _hasKit = state.cart[17] > 0;
  const _isEmergency = state.version === 'emergency' || _hasKit;
  if (subtotal > 0 && subtotal < 15 && !_isEmergency) {
  ...
  if (_firebaseFs && orderItems.length) {
    _firebaseFs.collection('orders').add({
      ...
      notes:        getOrderNotes(),
      nurse_assist: state.nurseAssist || false,
      is_emergency: _wasEmergency || false,
      created_at:   firebase.firestore.FieldValue.serverTimestamp()
    }).catch(e => console.warn('[Period.] Firestore order save failed:', e));
  }


  const _wasEmergency = state.version === 'emergency' || (state.cart[17] > 0);
  state.nurseAssist  = false;
  state.cart         = {};
  state.appliedPromo = null;
  updateCartBadge();
  renderCart();
...
function init() {
  ...
  $('heroOrderNow').addEventListener('click', () => {
    const isUrgent = state.version === 'emergency' ||
      ($('heroOrderNowText') && (
        $('heroOrderNowText').textContent.includes('Urgent') ||
        $('heroOrderNowText').textContent.includes('NOW')
      ));
    if (isUrgent) {
      addToCart(17);
      navigate('shop');
      setTimeout(() => { renderEmergencyShop(); openCart(); showToast('Emergency kit added ⚡ Tap Checkout when ready'); }, 200);
    } else {
      navigate('shop');
      setTimeout(() => {
        if ($('heroOrderNowText') && $('heroOrderNowText').textContent.includes('Natural')) {
          showProductsMode('holistic');
        } else { showCategoriesMode(); }
      }, 100);
    }
  });
...
function showQuizUrgencyCheck() {
  ...
  if (yesBtn) yesBtn.addEventListener('click', () => {
    ...
    setTimeout(() => {
      addToCart(17);
      navigate('shop');
      setTimeout(() => { renderEmergencyShop(); openCart(); showToast('Emergency kit added ⚡ Tap Checkout when ready'); }, 250);
    }, 200);
  });
